	...
	package lib {
		...
		*/
		...
		trait ProtoUser[T <: ProtoUser[T]] extends MongoRecord[T] with MongoId[T] {
			...
			def niceName: String = (firstName.value, lastName.value, email.value) match {
			...
			def shortName: String = (firstName.value, lastName.value) match {
		...
		trait MetaMegaProtoUser[ModelType <: MegaProtoUser[ModelType]] extends MongoMetaRecord[ModelType] {
			...
			override def fieldOrder = firstName :: lastName :: email :: locale :: timezone :: password :: Nil
			...
			*/
			...
			def emailFrom = "julien@tanjona.co.uk"
